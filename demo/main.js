// MotionKit demo behavior — split from index.html (styles in styles.css)
    // If the bundle failed to load/parse (e.g. an engine-specific issue), don't
    // let the whole page stay hidden behind the mk-preload veil — reveal it.
    if(typeof MotionKit==='undefined'){
      document.documentElement.classList.remove('mk-preload');
      throw new Error('MotionKit failed to load');
    }
    try{ MotionKit.config({smooth:false}); }catch(_){}
    MotionKitPlayground.capture(document);
    ...
    (()=>{
      ...
      let started=false;
      const startModules=()=>{ if(started)return; started=true; try{MotionKit.init(document);}catch(_){}
        document.documentElement.classList.remove('mk-preload'); };
      ...
      wordmark.className='mk-loader-wordmark';
      wordmark.textContent='MotionKit';
    ...
    const registered=new Set(Object.keys(MotionKit.registry));
    ...
    document.getElementById('module-list').addEventListener('click',(event)=>{
      ...
      if(!chip)return;
      const attr='data-mk-'+chip.dataset.module.replace(/[A-Z]/g,m=>'-'+m.toLowerCase());
    ...
    syncTheme();
    themeButton.addEventListener('click',()=>{const light=document.documentElement.classList.toggle('light');syncTheme();try{localStorage.setItem('mk-theme',light?'light':'dark')}catch(_){}});})();

    document.addEventListener('click',(event)=>{
      const button=event.target.closest('[data-action]');
      if(!button)return;
      if(button.dataset.action==='replay'){
        const el=document.querySelector(button.dataset.target); if(el) MotionKit.replay(el,button.dataset.module);
      }
      if(button.dataset.action==='replay-parent'){
        const card=button.closest('.card'); const el=card?.querySelector(`[data-mk-${button.dataset.module.replace(/[A-Z]/g,m=>'-'+m.toLowerCase())}]`); if(el) MotionKit.replay(el,button.dataset.module);
      }
    });
    ...
    const runPageReveal=(effect='curtain')=>{
      ...
      MotionKit.destroyModule(document.body,'pageReveal');
      const panelOptions=window.MotionKitPlayground?.pageRevealOptions?.()||{duration:.65,color:getComputedStyle(document.documentElement).getPropertyValue('--accent').trim()||'#ff5b1c',color2:'#101318'};
      MotionKit.pageReveal(document.body,{...panelOptions,effect});
    };
    ...
    const runLoader=(type='slot')=>{
      ...
      document.body.appendChild(overlay);
      let instance;
      instance=MotionKit.loader(overlay,{type,minDuration:1100,duration:.45,label:type==='bar'?'Loading assets':'',onComplete:()=>{instance?.destroy();overlay.remove();}});
    };
    document.getElementById('loader-demo')?.addEventListener('click',()=>runLoader('circular'));
    document.getElementById('smooth-on')?.addEventListener('click',()=>MotionKit.enableSmooth({duration:1.05}));
    document.getElementById('smooth-off')?.addEventListener('click',()=>MotionKit.disableSmooth());
    document.querySelectorAll('[data-slider-action]').forEach(button=>button.addEventListener('click',()=>{
      const slider=button.closest('.card').querySelector('[data-mk-slider]');
      const instance=MotionKit.getInstance(slider,'slider');
    ...
    (()=>{
      ...
      const apply=(lang)=>{
        ...
        window.MotionKitPlayground?.setHelpLang?.(lang);
        window.MotionKitPlayground?.refreshHelp?.();
      ...
      let saved='ko';
      try{saved=localStorage.getItem('mk-lang')||'ko';}catch(_){}
      select.value=saved;
      if(saved!=='ko')apply(saved);
      select.addEventListener('change',()=>{
        apply(select.value);
        try{localStorage.setItem('mk-lang',select.value);}catch(_){}
      });
    })();
    ...
    MotionKitPlayground.mount(document);

    // Reusable toast — window.mkToast(msg, {duration}). Multi-line via '\n',
    // always centered. Use it anywhere: copy, apply, reset, unsupported hints…
    (()=>{
      let host=null;
      window.mkToast=(msg,opts={})=>{
        if(!host){host=document.createElement('div');host.className='mk-toast-host';document.body.appendChild(host);}
      ...
      if(!canHaptic){
        document.querySelectorAll('[data-mk-vibrate]').forEach(btn=>{
          btn.addEventListener('click',()=>window.mkToast('이 환경에서는 진동(Haptic)이 지원되지 않습니다.\n주로 Android 기기에서 동작합니다.'),{passive:true});
        });
      }
    })();
